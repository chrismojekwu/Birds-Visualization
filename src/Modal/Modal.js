...
function Modal(props) {
  ...
  const renderConnection = () => {
    return props.connect.image.toString().includes("/") === true ? (
      <img
        src={props.connect.image}
        alt={props.connect.name}
        className="modal-img"
      />
    ) : (
      <div
        className="target"
        style={{ backgroundColor: props.connect.image }}
      ></div>
    );
  };
        ...
        <div className="modal-content">
          <div className="connect-imgs">
            {props.data.image.toString().includes("/") === true ? (
              ...
              <div
                className="source"
                style={{ backgroundColor: props.data.image }}
              ></div>
            )}
            ...
            {props.connect === "" ? (
              ...
              <div
                className="connect-line skinny"
                onMouseEnter={() => setOffset("-20px")}
                onMouseLeave={() => setOffset("150px")}
              >
                <div class="hover">Hover for Details</div>
              </div>
            )}
          ...
          <button id="close" onClick={() => props.close(false)}>
            Close
          </button>
        </div>
      </div>
  ...
  return props.show === false ? "" : renderData();
}
